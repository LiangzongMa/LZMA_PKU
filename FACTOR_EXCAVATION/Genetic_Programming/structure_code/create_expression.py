# Function of creating a function
import random
    ...
    while True:
        ...
        for i in range(operator_max):
            ...
            if len(expression) == 0:  # add operator to the expression for the first time
                for parameter in operator_information.keys():
                    ...
                    elif operator_information[parameter] == 'both':  # both matrix and number are available
                ...
                for parameter in operator_information.keys():
                    if operator_information[parameter] == 'matrix':  # add matrix
                        ...
                        # judge whether to choose a matrix from expressions before
        ...
        for string in inspect_temp:
            if string in operator:
                count += 1
        if operator_min <= count <= operator_max:
            break
    return ret

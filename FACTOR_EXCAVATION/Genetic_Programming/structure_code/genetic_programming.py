...
def gp(operator, matrix, number, operator_max, operator_min, survive_rate, generations, variation_rate, expression_number, ret):
    ...
    for generation in range(generations):
        ...
        for i in range(len(expression_collection)):
            ...
            expression_fitness = fitness_icir(eval(expression_collection[i]), ret, step=step)
            fitness_collection_all[expression_collection[i]] = expression_fitness
            expression_fitness = np.abs(expression_fitness)
            if expression_fitness > 1.5:  # record the expressions that perform extremely well during the process
                final_expressions.append(expression_collection[i])
            ...
            print('Time cost: %.2fs' % (end_time_fitness-start_time_fitness))
    ...
    if 'Factor_Expressions.csv' not in os.listdir('factor_result'):
        record = pd.DataFrame(columns=['GP_Index', 'Expression_Index', 'Expressions', 'IC_IR'])
    else:
    ...
    expression_collection = list(set(final_expressions+expression_collection))
    print('Finally, we get these expressions:')
    for i in range(len(expression_collection)):
        print(expression_collection[i])
        information = [gp_index, i + 1, expression_collection[i], fitness_collection_all[expression_collection[i]]]
        data_add = pd.Series(information, index=['GP_Index', 'Expression_Index', 'Expressions', 'IC_IR'])
